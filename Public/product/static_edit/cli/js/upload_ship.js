        ...
        if (!pd) {
            ...
            $(".edui-ship-file").upload({
                ...
                'maxsizefun': function(nowsize) { //超出大小回调
                    ...
                    $('#modal-alert').modal('show').find('.modal-title').text('上传的视频过大')
                    .end().find('.modal-body').empty().append(alertContent);              
                },
...
//                  alert('上传成功','ok');
                    ...
                    $("img[title = "+ imgTitle +"]").after("<iframe height=400 width=100% src='"+ re_data +"' frameborder=0 'allowfullscreen'></iframe>").remove();
                },
